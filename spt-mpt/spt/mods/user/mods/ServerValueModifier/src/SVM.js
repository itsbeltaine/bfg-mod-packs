...
class MainSVM
{
	 ...
	 postDBLoad(container)
	{
		...
		*/
		...
		"Sanitar did nothing wrong","I actually played tarkov, finally",
		...
		Logger.log(`SVM 1.8 has initialized, ` + funni[Math.floor(Math.random() * funni.length)],"blue");
		...
		if (Config.Loot.EnableLoot)
		{
			...
			locs.containerRandomisationSettings.enabled = !Config.Loot.Locations.RandomContainers
			//############## AIRDROPS SECTION ##################
		...
		if (Config.Services.EnableServices)
		{
			...
			Repair.armorKitSkillPointGainPerRepairPointMultiplier = Config.Services.RepairBox.RepairSkillPoint;
			Repair.applyRandomizeDurabilityLoss = !Config.Services.RepairBox.NoRandomRepair;
			Repair.maxIntellectGainPerRepair.kit = Config.Services.RepairBox.MaxSkillPointsPerRepair;
			Repair.maxIntellectGainPerRepair.trader = Config.Services.RepairBox.MaxSkillPointsPerRepair;
		...
		if (Config.Items.EnableItems)
		{
			...
			for (const id in items)
			{
				...
				if(base._parent == "5c99f98d86f7745c314214b3" && !(base._props.MaximumNumberOfUsage == 1 && !Config.Items.AvoidSingleKeys) && !(MarkedKeys.includes(base._id) && !Config.Items.AvoidMarkedKeys))
				{
					base._props.MaximumNumberOfUsage *= Config.Items.KeyUseMult
				}
				if(base._parent == "5c164d2286f774194c5e69fa" && base._props.MaximumNumberOfUsage !== undefined  && !(base._props.MaximumNumberOfUsage == 1 && !Config.Items.AvoidSingleKeys))
				{
		...
		{				
			...
			if(Config.Traders.Fence.FenceBlacklist !== "" || (Config.Traders.Fence.Blacklist).includes("\r\n"))
			{
